use nalgebra::{IsometryMatrix3, Matrix3, Matrix4, Matrix6, Rotation3, Translation3, Vector6};
...
pub(super) fn read_web(url: &str) -> std::io::Result<String> {
    ...
    let mut response = reqwest::blocking::get(url).map_err(std::io::Error::other)?;
    let mut contents = String::new();
        ...
        .map(|(_, (i, j, l))| {
            (
                ...
                Link {
                    ...
                    space_spatial_twist: Vector6::zeros(),

                    local_spatial_twist: Vector6::zeros(),

                    // construct from joint
                    // local_spatial_twist: None,
                    global_zero_pose: Matrix4::identity(),
                    parent_zero_pose: Matrix4::identity(),
...
pub(crate) fn pose_to_se3(pose: &urdf_rs::Pose) -> Matrix4<f64> {
    let rpy = pose.rpy;
    let xyz = pose.xyz;
    let i = IsometryMatrix3::from_parts(
...
pub(super) fn fullfill_link_map(
    ...
    for link in bfs {
        ...
        if let Some(parent_index) = parent_index {
            let parent_global_pose = link_map.get(&parent_index).unwrap().global_zero_pose;
            ...
            let global_twist = global_pose.lee_adjoint() * twist;
            link_mut.global_zero_pose = global_pose;

            link_mut.local_spatial_twist = twist;
            link_mut.space_spatial_twist = global_twist;
        }
    }
}

pub(super) fn joint_twist(joint: &urdf_rs::Joint) -> Vector6<f64> {
    match joint.joint_type {
        urdf_rs::JointType::Revolute | urdf_rs::JointType::Continuous => {
            ...
            Vector6::new(norm_x, norm_y, norm_z, 0., 0., 0.)
        }
        ...
        urdf_rs::JointType::Prismatic => {
            ...
            let norm_y = joint.axis.xyz[1] / norm;
            let norm_z = joint.axis.xyz[2] / norm;
            Vector6::new(0., 0., 0., norm_x, norm_y, norm_z)
        }

        urdf_rs::JointType::Fixed => Vector6::zeros(),
...
mod tests {
    ...
    fn fullfill_link_map_test() {
        ...
        link_map.iter().for_each(|(i, l)| {
            println!(
                "{}: {:.3} {:.3} {:.3}",
                i, l.global_zero_pose, l.space_spatial_twist, l.local_spatial_twist
            );
        });
    }
}
